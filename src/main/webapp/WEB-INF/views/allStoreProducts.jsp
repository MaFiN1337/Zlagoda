...
<table>
    ...
    <tr>
        <th>#</th>
        <th><fmt:message key="zlagoda.UPC_Prom" bundle="${rb}" /></th>
    ...
    <tbody>
    <c:forEach items="${storeProducts}" var="storeProduct" varStatus="status">
        <tr>
            <td>${status.index + 1}</td>
            <td>${storeProduct.getUpc_prom()}</td>
            <td>${storeProduct.getSelling_price()}</td>
            <td>${storeProduct.getProducts_number()}</td>
            <td>${storeProduct.getPromotional_product()}</td>
            <td>${storeProduct.getProduct().getName()}</td>
            <td>
                <a href="./storeProducts/updateStoreProduct?UPC=${storeProduct.getUPC()}">
                    <fmt:message key="zlagoda.update" bundle="${rb}" />
                </a>
                <br>
                <a href="./storeProducts/deleteStoreProduct?UPC=${storeProduct.getUPC()}">
