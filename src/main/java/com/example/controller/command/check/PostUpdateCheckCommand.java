...
public class PostUpdateCheckCommand implements Command {

    private final CheckService checkService;
    private final Customer_cardService customerCardService;
    private final EmployeeService employeeService;

    public PostUpdateCheckCommand(CheckService checkService, Customer_cardService customerCardService, EmployeeService employeeService) {
    ...
    private Check getUserInput(HttpServletRequest request) {
        return new Check.Builder().setNumber(request.getParameter(Attribute.CHECK_NUMBER))
                .setNumber(request.getParameter(Attribute.CHECK_NUMBER)).setPrint_date(LocalDateTime.parse(request.getParameter(Attribute.PRINT_DATE)))
