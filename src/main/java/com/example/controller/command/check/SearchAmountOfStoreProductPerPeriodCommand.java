...
public class SearchAmountOfStoreProductPerPeriodCommand implements Command {

    private final CheckService checkService;

    public SearchAmountOfStoreProductPerPeriodCommand(CheckService checkService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)
        ...
        Optional<CheckSumDto> sum = checkService.searchAmountOfStoreProductsPerPeriod(
                new Store_product.Builder().setUpc(UPC).build(),LocalDate.parse(fromDate), LocalDate.parse(toDate));
