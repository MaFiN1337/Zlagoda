...
public class PostAddCustomerCardCommand implements Command {
    ...
    public PostAddCustomerCardCommand(Customer_cardService customerCardService) {
    ...
    private Customer_card getUserInput(HttpServletRequest request) {
        return new Customer_card.Builder().setName(request.getParameter(Attribute.CUST_NAME))
    ...
    private void redirectToAllCustomerCardsPageWithSuccessMessage(HttpServletRequest request, HttpServletResponse response)
        ...
        urlParams.put(Attribute.SUCCESS, Message.SUCCESS_CUSTOMER_CARD_ADDITION);
